...
- 产物导入失败保留可诊断的原因，区分缺失 manifest、非法格式/schema 与必需产物缺失；补齐 JSON 序列化提示和历史租约冲突的人工处置说明，保持严格校验与原恢复边界。
- 明确发布问题的影响与阻断条件，在默认入口披露本轮 Codex/DSH 真实验收缺口；补丁不承诺模型永远生成合法产物，也不自动裁决历史租约冲突。
