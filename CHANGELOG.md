...
- 真实 provider `test-report`/`ac-evidence` artifact 协议明确要求 `criteriaEvidence[].criterionId/status/evidence`，`test-report.summary` 如存在必须是字符串；若 Codex 写出对象形式 `summary`、`criteriaEvidence[].id/evidenceSummary/coverage` 或 `passed_with_*`/`failed_with_*`/`blocked_with_*` 状态标签，Tekon 会窄归一化为 schema 所需字段，缺失状态、含糊状态或只有 `criterion` 而无证据字段仍失败，避免 QA validation 已产出有效证据但因字段命名漂移中断。
