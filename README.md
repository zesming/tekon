...
- 工作可用化增量已验证：`repo-profile.yaml` 仓库画像、角色 prompt 注入、`--agent claude-code` adapter 接线、`delivery prepare` PR 准备包、`eval readiness` 工作就绪度评估。
- 尚未作为已完成能力发布：真实远端 PR 创建、自动 merge、生产级真实 LLM workflow 稳定性和远端发布流程。
...
`eval readiness` 会检查 workflow、audit、验证 gate、delivery package、PR 准备包和 pending human gate，作为“这次是否可拿去人工审阅/提交”的最小评估面。
...
当前状态是本地 V2 重构验收通过，不是公开生产发布。任何对外说明都应明确：Donkey 现在已可在本地通过 CLI 跑 mock workflow、dynamic dry-run、delivery dry-run、delivery prepare、eval readiness 和 Web dashboard human approval；真实远端 PR 创建、自动 merge 和生产级真实 LLM workflow 稳定性仍需后续发布范围确认。
