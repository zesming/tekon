...
| Provider 接入     | 已实现 mock、Claude Code 与 Codex adapter 协议 | 真实 provider 通过 artifact manifest 交付结构化产物；Codex 走本机 `codex --profile internal ... exec`，默认真实 provider 总超时为 1 小时、无输出或产物进展超时为 15 分钟，并写入 progress JSON。非 `code-changes` 节点若修改源码会被 Engine 拦截。                    |
...
| 交付准备          | 已实现受控流程                                 | `delivery dry-run` 只生成计划，`delivery prepare` 先通过 pre-PR readiness 再生成本地 PR 包，`delivery create-pr --approve-human` 才产生远端副作用；受控 `git/gh` 命令和 create-pr 前置只读 probe 使用同一 1 小时总超时、15 分钟无输出或产物进展超时和 progress JSON。 |
