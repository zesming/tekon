...
| 评审类 artifact schema 不合法              | 缺少 `reviewScope`、`reviewProcess`、`decision`，`findings` 不是数组，或 finding 条目的 `severity`、`message` 不合法。                                                               | 按节点 prompt 中的 role-scoped review JSON 示例重写 artifact；不要用 `reviewRole`、`reviewedArtifacts` 或数组/对象形式的 `reviewScope` 替代 schema 字段。                            |
