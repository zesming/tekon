...
如果 Codex 在写完有效 manifest 后没有及时退出，adapter 会尝试读取并校验 `$TEKON_ARTIFACT_MANIFEST` 指向的文件；只要 workflow 必需 artifact 已完整入库，该节点会继续进入后续 gate。若 manifest 缺失、schema 非法或必需 artifact 不齐，该节点仍会失败。为兼容真实 Codex 误写出的字面文件名，adapter 也会在受控 `TEKON_OUTPUT_DIR` 内按同一 schema 检查 `TEKON_ARTIFACT_MANIFEST` 文件；该兼容路径不改变 provider prompt 的推荐写法。
...
| `agent failed: provider=codex exitCode=1` | Codex 没写 manifest、artifact schema 不合法，命令执行失败，或用户 args 试图覆盖 profile、sandbox、approval、文件系统、配置或危险 bypass。 | 查 `.tekon/runs/<runId>/<nodeId>/` 下 stdout/stderr、`artifact-manifest.json`、字面 `TEKON_ARTIFACT_MANIFEST` 和 artifact 内容。    |
