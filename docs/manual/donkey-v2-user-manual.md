...
初始化会创建 `.donkey/` 目录、`config.yaml`、`repo-profile.yaml`、SQLite 数据库、角色目录、workflow 目录、worktree 目录和 eval 目录。`.donkey/` 是运行态目录，默认不提交；重要验收结论、run id、Gate 结果和风险判断应同步写入 `docs/reviews/`。

`repo-profile.yaml` 会根据目标仓库 `package.json` 自动填充可识别的 `build`、`typecheck`、`lint`、`test`、`e2e`、`security` 命令，并保留 PR base branch 和风险路径配置。内置 workflow 的 build/lint/test/security gate 会通过 `commandRef` 读取仓库画像命令；缺失 build/lint/test 命令时 gate 不会静默跳过，而会以 missing-command 失败。运行前可用 `workflow preflight` 查看缺失项；如果 `package.json` 中存在 `compile`、`bundle`、`test:e2e`、`playwright` 等候选脚本，preflight 会提示建议写入 `.donkey/repo-profile.yaml` 的命令。确实不适用的普通命令 gate 可在 repo profile 中显式配置 `notApplicable: true` 和 `reason`；运行时会记录为 `skipped/not-applicable`，readiness 会把它视为已显式解决，PR 包会保留 skipped 证据。`security-scan` 没有外部命令时仍执行 Donkey 内置扫描，不应通过 `notApplicable` 绕过基础安全扫描。
...
```bash
node /path/to/donkey/packages/cli/dist/index.js demand shape "给 Web dashboard 增加需求塑形入口，要求 e2e 通过" --write --repo /path/to/project
node /path/to/donkey/packages/cli/dist/index.js demand approve /path/to/project/.donkey/demands/<shapeId>.json
node /path/to/donkey/packages/cli/dist/index.js eval demand-shape /path/to/project/.donkey/demands/<shapeId>.json
node /path/to/donkey/packages/cli/dist/index.js run --demand-file /path/to/project/.donkey/demands/<shapeId>.json --agent mock --repo /path/to/project
```

`demand shape --write` 会在 `.donkey/demands/` 下写入 JSON 源文件和 Markdown 审阅稿，内容包括需求分类、推荐模板、风险等级、风险标签、非目标、假设、开放问题和验收标准。`run --demand-file` 默认要求该需求文件已被 `demand approve` 标记为 approved；如果仍有开放问题，人可以选择先补充需求，也可以在明确接受风险后批准继续。需要记录具体操作者时，可额外传入真实账号或姓名。`eval demand-shape` 会检查标题、验收标准、非目标、风险边界和开放问题是否已解决或已被人工批准。
...
输出会列出每个 node/gate 的 `commandRef`、解析状态和最终命令。`security-scan` 在没有外部安全命令时会显示 Donkey 内置扫描。普通命令缺失时，输出会包含 `status=missing`、`profilePath`、`hint`；若能从 `package.json` 找到候选脚本，还会包含 `suggestedScript` 和 `suggestedCommand`。示例：某仓库只有 `compile` 脚本但没有 `build` 脚本时，preflight 会提示 `suggestedCommand=npm run compile`，你需要确认后把 `commands.build` 写入 `.donkey/repo-profile.yaml`。
...
```bash
node /path/to/donkey/packages/cli/dist/index.js approval summary --run-id <runId> --repo /path/to/project
node /path/to/donkey/packages/cli/dist/index.js eval approval-summary --run-id <runId> --repo /path/to/project
node /path/to/donkey/packages/cli/dist/index.js resume --run-id <runId> --approve-human --repo /path/to/project
node /path/to/donkey/packages/cli/dist/index.js approval reject --run-id <runId> --decision-id <decisionId> --repo /path/to/project
```
...
PR 创建后，可以只读查询远端 CI/checks 状态并写回 Donkey evidence：

```bash
node /path/to/donkey/packages/cli/dist/index.js delivery ci-status --run-id <runId> --repo /path/to/project
```

如果 run 尚未落库 PR URL，或你想查询指定 PR/分支，可显式传入 selector：

```bash
node /path/to/donkey/packages/cli/dist/index.js delivery ci-status --run-id <runId> --selector <prUrl|branch> --repo /path/to/project
```
...
```bash
node /path/to/donkey/packages/cli/dist/index.js delivery ci-watch --run-id <runId> --max-attempts 20 --interval-ms 15000 --repo /path/to/project
```
...
`eval readiness` 判断单个 run 能不能进入人工审阅；`eval work-usability` 判断一组真实工作样本是否达到试用门槛。样本清单建议保存在目标仓库 `.donkey/eval/work-usability-samples.yaml` 或可提交的 `docs/reviews/` 附件中。
...
- 每个样本是否有 worktree lease、worktree 路径是否位于 `.donkey/worktrees`、lease 是否释放。
...
```bash
node /path/to/donkey/packages/cli/dist/index.js review --run-id <runId> --repo /path/to/project
```
...
```bash
node /path/to/donkey/packages/cli/dist/index.js review --run-id <runId> --repo /path/to/project --json
```
...
命令 stdout/stderr 由 CommandGateway 写入 `.donkey/runs/<runId>/commands` 或 gate 输出目录。写入前会对明显密钥模式做脱敏；脱敏后的内容仍保留上下文和失败信息，便于排查 gate 失败。Audit event payload 不应写入 API key、token、生产凭证或环境变量值。
...
node /path/to/donkey/packages/cli/dist/index.js delivery ci-status --run-id <runId> --repo /path/to/fixture
node /path/to/donkey/packages/cli/dist/index.js delivery ci-watch --run-id <runId> --max-attempts 1 --interval-ms 0 --repo /path/to/fixture
```
