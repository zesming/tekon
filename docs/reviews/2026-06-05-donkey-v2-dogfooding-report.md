# Donkey V2 Dogfooding Report

日期：2026-06-05
范围：Phase 3 Task 19，Donkey-on-Donkey 本地自举验收
...
| fixture repo     | `/tmp/donkey-dogfood-9s5Nno`                                                                |
...
```bash
mktemp -d /tmp/donkey-dogfood-XXXXXX
git init
npm init -y
...
git commit -m init
node packages/cli/dist/index.js init --repo /tmp/donkey-dogfood-9s5Nno
npm exec --yes -- pnpm@10.12.1 exec node packages/cli/dist/index.js run --dynamic --dry-run "整理 Donkey V2 Phase 3 验收材料，高风险数据变更" --agent mock --repo /tmp/donkey-dogfood-9s5Nno
npm exec --yes -- pnpm@10.12.1 exec node packages/cli/dist/index.js run "整理 Donkey V2 Phase 3 验收材料" --template standard-feature --agent mock --repo /tmp/donkey-dogfood-9s5Nno
npm exec --yes -- pnpm@10.12.1 exec node packages/cli/dist/index.js delivery dry-run --run-id run_7a795092-5927-4560-851b-77e102671f86 --repo /tmp/donkey-dogfood-9s5Nno
npm exec --yes -- pnpm@10.12.1 exec node packages/cli/dist/index.js run "修复 Donkey V2 Phase 3 验收材料中的人工确认路径" --template bugfix --agent mock --repo /tmp/donkey-dogfood-9s5Nno
npm exec --yes -- pnpm@10.12.1 exec node packages/cli/dist/index.js resume --run-id run_40091657-33de-4b53-b745-ea622a1004e0 --approve-human --repo /tmp/donkey-dogfood-9s5Nno
```
...
- 错误 token 被拒绝，正确 `.donkey/web-session.json` token 可批准 pending human gate。
