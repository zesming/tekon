...
- `donkey run --template standard-feature --agent mock`：通过 CLI e2e 覆盖，结果 `status=passed`，本地 artifacts 生成。
- `donkey run --dynamic --dry-run --agent mock`：通过 CLI e2e 覆盖，输出 `dryRun=true` 和 constraint mutation，包括 `conditional-high-risk-human-gate`、`conditional-rollback-plan`。
...
- Review record：本文件和 `2026-06-05-donkey-v2-phase2-cli-evidence.md` 已保存到 `docs/reviews/`，并提供 HTML 审阅版。
