...
| `npm exec --yes -- pnpm@10.12.1 test -- --run`                                             | 通过；46 test files / 134 tests。该命令不启用 coverage        |
| `npm exec --yes -- pnpm@10.12.1 exec vitest --exclude "**/__manual__/**" --run --coverage` | 通过；46 test files / 134 tests，覆盖率见下节                 |
| `npm exec --yes -- pnpm@10.12.1 --filter @tekon/core test:e2e`                             | 通过；6 test files / 7 tests                                  |
| `npm exec --yes -- pnpm@10.12.1 --filter @tekon/cli test:e2e`                              | 通过；2 test files / 2 tests                                  |
| `npm exec --yes -- pnpm@10.12.1 --filter @tekon/web test:e2e`                              | 通过；2 Chromium tests                                        |
...
| 范围                             | 目标         | 实际         |
| -------------------------------- | ------------ | ------------ |
| All files                        | 无全局硬阈值 | 91.00% lines |
| `packages/core/src`              | >= 80% lines | 90.92% lines |
| `packages/cli/src`               | >= 70% lines | 91.98% lines |
| `packages/web/src/server` 与 API | >= 70% lines | 88.45% lines |
...
| Unit/integration tests | 通过；coverage 命令 46 files / 134 tests                                                                                |
...
| Human gate             | CLI bugfix run 可 pending 后 approve；Web dashboard 可用 session token approve，并展示 command/gate/request/risk 上下文 |
| Audit hash             | Dogfooding delivery run 与 human gate run 均 `audit.valid=true`；Web audit 支持 node/gate/role filter                   |
...
- 本地 CLI 可初始化项目、运行 mock template、执行 dynamic dry-run、显式控制 dirty base、处理 human gate、查看状态和日志、清理 worktree。
- Core 支持 SCM delivery dry-run、delivery evidence package、metrics extraction 和 Markdown/HTML report 生成。
- Web dashboard 可显式绑定项目根，展示 overview/artifacts/gates/audit/roles/workflows/settings、human gate command/gate/request/risk 上下文和 audit hash/filter，并通过 `tekon init` 生成的 `.tekon/web-session.json` token 执行 human approval。
