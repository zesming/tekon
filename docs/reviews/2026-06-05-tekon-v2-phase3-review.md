...
2. `packages/web/src/server/api/root.ts` 中 `project.clean` 同样只校验 token，没有校验 run scope。虽然删除路径位于当前 `.tekon/runs/<runId>`，但仍属于写操作，应绑定 project scope。

## 建议修复项

1. `tekon init` 原先不生成 `.tekon/web-session.json`，而 Web 写操作依赖该 token。真实用户首次使用 Web human approval 缺少明确生成路径。
...
- `tekon init` 生成 `.tekon/web-session.json`，token 为 32 bytes hex。
...
- `@tekon/web` `write-auth.test.ts`：2 tests passed
- `@tekon/cli` `run-cli.test.ts`：1 test passed
- `git diff --check`
- 高置信敏感 token 模式扫描
