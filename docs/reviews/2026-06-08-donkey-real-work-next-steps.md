...
| 审批与通知               | Web 可处理 human gate，CLI 可 resume；但还偏本地按钮/命令                                               | 生成可复制审批摘要，包含风险、命令、影响文件、证据链接、批准/拒绝命令；后续再接飞书 IM                                  | 先做 CLI/Web approval summary，不急着做完整飞书通知机器人                                  |
...
1. **审批摘要和通知入口**：让人能在飞书或 Web 中快速判断批准/拒绝。
