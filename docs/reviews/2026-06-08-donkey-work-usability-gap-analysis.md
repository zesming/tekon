...
| Web 产品面        | 能审阅最新 run、处理 human gate，并查看 readiness、diff、artifact 正文、gate logs 和 PR 包 | 仍不能从 Web 发起 run、执行 prepare/create-pr 或管理多 run 审阅流。                    |
...
当前状态：2026-06-08 本轮已实现第一版 review surface：core 聚合 readiness、PR body/package、delivery diff、artifact 正文、gate log 和下一步命令；CLI 新增 `review --run-id`；Web 新增 Readiness、Diff、Artifact 正文、Gate Logs、PR 包和下一步区块。仍需补 Web 发起 run、prepare/create-pr 操作和 artifact/gate/audit 互跳体验。
...
4. 用真实 run 验证第一版 review surface 是否足够 5 分钟决策，补 artifact/gate/audit 互跳和 Web prepare/create-pr 操作。
...
- P0-5：新增 core review surface、CLI `review --run-id` 和 Web 审阅区，聚合 readiness failed checks、PR body/package、delivery diff、artifact 正文、gate log 和下一步命令。
...
仍未完成并继续保留为后续 P0/P1 工作：真实仓库端到端样本集、Web 发起 run/prepare/create-pr、artifact/gate/audit 互跳体验、至少 2 次真实测试 PR 证据、生产级隔离和真实 provider 长期稳定性数据。
