...
| Web Dashboard            | `packages/web/src/client/App.tsx`、`packages/web/src/server/api/root.ts`                                          | 本地 dashboard 可看 overview/artifacts/gates/audit/roles/workflows/settings，并批准/拒绝 human gate。                   |
...
事实：Web 当前展示 artifact 列表、gate 列表、audit 摘要和 human gate；artifact 只显示 summary/path，不显示正文；没有 diff viewer、gate log viewer、readiness 结果、PR 包内容和下一步命令。
