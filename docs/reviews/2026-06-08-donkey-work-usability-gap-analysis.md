...
3. workflow gate 已开始使用 `.donkey/repo-profile.yaml` 的仓库命令，`workflow preflight` 已能提示缺失命令修复方向；后续要用非 pnpm 或脚本名不同的真实仓库验证。
...
- P0-3：内置 workflow 改为 `commandRef` 引用 `.donkey/repo-profile.yaml`，CLI 新增 `workflow preflight` 展示 gate 命令解析结果，在缺失命令时提示 repo profile 写入位置和候选脚本命令，并支持普通 command gate 的显式 `notApplicable`。
