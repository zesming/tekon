...
| 仓库画像          | init 可生成 `repo-profile.yaml`，内置 workflow gate 通过 `commandRef` 解析画像命令，`workflow preflight` 会对缺失命令给出修复引导和候选脚本建议 | 已从硬编码模板命令推进到画像驱动，并补齐第一版缺失命令引导；仍缺真实非 pnpm 仓库证据和“显式不适用”的产品语义。 |
...
当前状态：2026-06-08 本轮已将内置 workflow 改为 `commandRef`，新增 `workflow preflight`，并补齐第一版缺失命令修复引导。缺失普通命令时，preflight 会输出 `status=missing`、`profilePath`、`hint`；如果 `package.json` 中存在 `compile`、`bundle`、`test:e2e`、`playwright` 等候选脚本，会输出 `suggestedScript` 和 `suggestedCommand`。后续仍需在真实非 pnpm 仓库验证，并设计“不适用”命令的显式配置语义，不能靠静默跳过 gate。
...
- P0-3：内置 workflow 改为 `commandRef` 引用 `.donkey/repo-profile.yaml`，CLI 新增 `workflow preflight` 展示 gate 命令解析结果，并在缺失命令时提示 repo profile 写入位置和候选脚本命令。
