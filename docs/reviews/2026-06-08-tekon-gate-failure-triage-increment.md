...
真实工作中，Tekon 的失败体验不能只停留在“某个 gate failed”。用户需要快速知道失败属于命令缺失、退出码失败、安全扫描、artifact schema 问题还是人工审批阻塞，并能跳到对应日志，判断是该修 repo profile、修代码、补 artifact，还是人工审批后继续。
...
- `missing-command` 仍要求用户显式修正 `.tekon/repo-profile.yaml` 或配置 `notApplicable`；Tekon 不会因为命令缺失静默跳过 gate。
...
- `review/surface.test.ts` 覆盖 `exit-code` 失败 gate 输出 `retry=after-fix`、日志锚点和 `tekon log --run-id` 建议命令。
...
- `security-findings` 仍保持 `after-fix`，建议命令为 `tekon review --run-id ...`，不提示绕过安全扫描。
