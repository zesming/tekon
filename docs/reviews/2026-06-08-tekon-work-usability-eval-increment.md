...
| CLI 样本评估入口         | `packages/cli/src/index.ts`                                                                                                                        | 新增 `eval work-usability --samples`，默认样本路径为 `.tekon/eval/work-usability-samples.yaml`。                          |
| 初始化 eval 目录         | `packages/cli/src/index.ts`                                                                                                                        | `tekon init` 会创建 `.tekon/eval`，用于保存本地样本清单。                                                                 |
...
- worktree lease 是否存在、路径是否位于 `.tekon/worktrees`、lease 是否释放。
...
- `docs/manual/tekon-v2-user-manual.md/html`：新增工作可用样本评估章节。
- `docs/reviews/2026-06-08-tekon-work-usability-gap-analysis.md/html`：更新 P0-2/P0-6/P0-7 当前状态。
- `docs/reviews/2026-06-08-tekon-work-usable-increment.md/html`：补充本轮能力和验证记录。
...
npm exec --yes -- pnpm@10.12.1 --filter @tekon/cli test:e2e -- --run packages/cli/__tests__/e2e/release-flow.test.ts
