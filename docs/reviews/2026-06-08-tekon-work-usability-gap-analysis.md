...
范围：综合 5 份飞书产品方案、本地实现、README/CHANGELOG、V2 用户手册、Phase 3/Dogfooding/Final Acceptance/工作可用化增量报告，判断 Tekon 要成为“我能在真实工作中使用的工具”还需要补齐什么。本文不讨论全面自动化，不把自动 merge、自动上线、生产权限变更、跨团队多租户平台作为近期目标。
...
| ---------- | ------------------------------------------------------------------------------- | ---------- | --------------------------------------------------------------------------- |
| 飞书文档 1 | `[Raw]Tekon-智能化研发流程`                                                     | 2026-06-08 | 提取原始设想：人负责需求澄清，后续由角色 Agent 和 workflow 推进。           |
| 飞书文档 2 | `[Beta-G]Tekon：面向技术基建团队的 AI 自动交付系统产品方案`                     | 2026-06-08 | 作为近期收敛版本，重点采用“技术基建/B-D 类需求/自动到 PR/证据包/高危受控”。 |
| 飞书文档 3 | `[Alpha-Q]Tekon — AI Agent 驱动的智能化研发流程产品方案`                        | 2026-06-08 | 参考完整链路、角色 Agent、人工介入和风险判断。                              |
| 飞书文档 4 | `[Alpha-G]Tekon：AI Native 产研流程执行系统完整产品方案`                        | 2026-06-08 | 参考完整远景对象模型、八大模块、项目驾驶舱、知识沉淀和分阶段规划。          |
| 飞书文档 5 | `[Alpha-D]Tekon — 智能化研发流程 · 产品方案`                                    | 2026-06-08 | 参考角色五元组、工具/技能体系、四级人在环路和全链路审计。                   |
...
Tekon 当前已经从概念推进到“本地受控 workflow 骨架可跑”的阶段：CLI/Web、SQLite 状态、Artifact/Audit、Gate、worktree、PR 准备包、受人工批准的 PR 创建、readiness 评估都有代码和测试支撑。

但它离“真实工作可用”还差一层证据和体验：真实仓库端到端稳定性、可审阅体验、真实 PR 证据、安全隔离证据和最小数据闭环仍不足。本轮已经补齐真实 provider artifact 协议、repo profile 驱动 gate、provider 快照恢复、PR 创建后的远端 CI 状态回写的第一版代码；下一阶段不应该继续扩展远景角色或上线链路，而应先把“给一个真实内部工具仓库的小需求，Tekon 能受控产出可审 PR，人在 5 分钟内判断能不能接受”打实。

优先级最高的补齐方向是：

1. 当前已接线的真实 provider 是 Claude Code；本轮已提供 Tekon artifact manifest 协议，后续需要真实仓库样本证明 provider 能稳定按协议产出结构化 artifacts。Codex 目前属于后续或自定义 provider，同样要先遵守 artifact 协议后才能声明可用。
...
3. workflow gate 已开始使用 `.tekon/repo-profile.yaml` 的仓库命令，`workflow preflight` 已能提示缺失命令修复方向；后续要用非 pnpm 或脚本名不同的真实仓库验证。
...
6. PR 创建后已能通过 `delivery ci-status` 只读查询 GitHub PR checks 并落库；`delivery ci-watch` 已补齐只读轮询和退避等待，后续仍需要真实 PR 上的 CI 证据、无 checks 细分提示和非 GitHub provider 边界。
...
| Artifact Store           | `packages/core/src/artifact/store.ts`、`packages/core/src/artifact/schemas.ts`                                    | artifact 版本化写入 `.tekon/runs`，schema 支持验收标准、criteria evidence、安全发现和 `ci-status`。                                                               |
...
| Worktree 隔离            | `packages/core/src/runtime/worktree-manager.ts`                                                                   | 节点在 git worktree 中执行，变更可提交并推进到 `tekon-delivery/<runId>`。                                                                                         |
...
| 远端 CI 状态             | `packages/core/src/delivery/ci-status.ts`、`packages/core/src/delivery/evidence.ts`                               | `delivery ci-status` 可只读调用 `gh pr checks`，把 PR checks 汇总状态写入 `ci-status` artifact 和审计事件；`delivery ci-watch` 可重复查询直到终态或达到次数上限。 |
...
| 远端 CI 状态      | 代码支持 `gh pr checks` 查询、fake fixture、artifact/evidence/readiness 入库，并新增 `delivery ci-watch` 轮询终态能力                                                                   | 仍缺真实 PR 上的 CI 证据；当前只覆盖 GitHub CLI，不覆盖其它 Git host 或 CI 聚合。                      |
...
| 仓库画像          | init 可生成 `repo-profile.yaml`，内置 workflow gate 通过 `commandRef` 解析画像命令，`workflow preflight` 会对缺失命令给出修复引导和候选脚本建议，普通命令 gate 支持显式 `notApplicable` | 已从硬编码模板命令推进到画像驱动，并补齐第一版缺失命令引导和显式不适用语义；仍缺真实非 pnpm 仓库证据。 |
...
修复前事实：`createMockAgentAdapter` 会直接写结构化 artifacts；`createClaudeCodeAdapter` 仅执行 Claude CLI 并返回 stdout/stderr 路径，没有把 `TEKON_OUTPUT_DIR`、artifact schema、所需 artifact 类型和写入协议明确传给 provider，也没有把 provider 输出解析为 artifact store 记录。
...
- Engine 在 prompt 和环境变量中明确声明 `TEKON_OUTPUT_DIR`、本节点必须产出的 artifact 类型、每类 artifact 的 JSON/YAML/Markdown schema。
...
当前状态：2026-06-08 后续增量已新增 work usability sample set schema 和 CLI `eval work-usability --samples`。默认阈值为 10 个样本、5 个 ready run、5 个真实 provider run、2 个真实 PR，并要求所有样本有隔离证据；fixture 可降低阈值回归测试。本轮又补齐 `eval work-usability record` 和 `--report-md/--report-html`，让真实 run 可以沉淀到样本清单并生成可提交报告；真实仓库样本本身仍未补齐。
...
影响：真实仓库只要不是 pnpm 或脚本名不同，Tekon 就会在 gate 层失败；这会把“工具不适配”误判成“需求实现失败”。

最低目标：

- `tekon init` 后提供 gate preflight，列出将运行的 build/typecheck/lint/test/e2e/security 命令。
- Workflow gate 支持引用 repo profile，例如 `commandRef: build`、`commandRef: test`。
- 缺失命令时不默认跳过；要求用户在 repo profile 中确认替代命令或显式标记“不适用”。

当前状态：2026-06-08 本轮已将内置 workflow 改为 `commandRef`，新增 `workflow preflight`，并补齐第一版缺失命令修复引导和显式不适用语义。缺失普通命令时，preflight 会输出 `status=missing`、`profilePath`、`hint`；如果 `package.json` 中存在 `compile`、`bundle`、`test:e2e`、`playwright` 等候选脚本，会输出 `suggestedScript` 和 `suggestedCommand`。用户确认不适用时可在 repo profile 中配置 `notApplicable: true` 和 `reason`；普通 command gate 会记录为 `skipped/not-applicable`，readiness 会视为显式解决，PR 包保留 skipped 证据。后续仍需在真实非 pnpm 仓库验证。
...
修复前本地依据：`packages/cli/src/index.ts` 的 `commandResume` 固定创建 `createMockAgentAdapter()`；`packages/web/src/server/api/root.ts` 会从 `.tekon/config.yaml` 读 `defaultAgent`，但 run 自身没有落库 provider 快照。CLI 和 Web 的恢复口径不一致。当前已落库 run provider/config snapshot，并在 CLI/Web resume 前校验。
...
影响：用户仍需要回终端翻 `.tekon` 文件、手动找 diff 和日志；这不满足“真实工作中 5 分钟判断是否接受”的目标。
...
当前状态：`eval work-usability --samples` 已能把 created PR 数和样本级 `expectedPrUrl` 纳入评估；本地 fixture 已覆盖 fake `gh` 的 create-pr 路径。仍缺至少 2 次真实受控远端 PR 证据。
...
当前状态：`eval work-usability --samples` 已检查 worktree lease 是否存在、路径是否位于 `.tekon/worktrees`、lease 是否释放，以及发生 push/PR 时是否有 `approvedBy`/`approvedAt`。这只是工作可用样本级隔离证据，不等于生产级 OS/container 沙箱或完整密钥治理。
...
| 缺口               | 当前状态                                                                                                                                           | 建议目标                                                                                                       |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| 需求塑形入口       | 已新增 CLI/Web 需求塑形和批准入口：`demand shape/approve` 生成需求卡、风险、非目标、开放问题和验收标准，`run --demand-file` 默认要求 approved      | 后续可接入真实 PM/LLM 需求澄清，但当前先保持确定性启发式和人工批准，不把开放问题自动当作已解决。               |
| Workflow 选择      | 已新增受控模板 `standard-feature`、`bugfix`、`test-improvement`、`docs-update`、`plan-only`，并提供 `workflow select` 与 `eval workflow-selection` | 动态规划仍先作为建议，不自动变成可执行 workflow；后续可补人工确认后保存模板的产品流。                          |
...
| 通知与审批         | 已新增 CLI/Web 可复制审批摘要，包含风险、命令、影响文件状态、证据入口和批准/拒绝入口；Web 本地处理 human gate，CLI 支持 approve/reject             | 用真实审批样本验证摘要质量；后续再接飞书 IM 通知，不绕过 human gate。                                          |
...
| 文档验收状态一致性 | 本轮已更新 `docs/reviews/2026-06-08-tekon-work-usable-increment.md` 和 HTML 审阅版                                                                 | 后续每次实现后仍需同步 Markdown/HTML，并在最终 reviewer 复查后写入正式结论。                                   |
...
2. 用 1-2 个非 pnpm 或脚本名不同的仓库验证 repo profile gate preflight，确认缺失命令修复引导能覆盖真实脚本差异。
...
Tekon 现在已经具备“工作可用工具的骨架”，但还不是“可以直接依赖的工作工具”。最短路径不是扩大自动化范围，而是把真实 provider、真实仓库、真实 PR、真实证据和真实恢复打穿。

只要 P0 补齐，Tekon 就可以进入一个合理的试用边界：在 2-3 个相似内部工具仓库中处理低中风险 B/D 类需求，默认推进到 PR 包或测试 PR，由人审阅、决定合入，并把失败原因沉淀为 repo profile、角色规则和 workflow 模板。

## 10. 2026-06-08 实施进展

本轮已完成 P0-1/P0-3/P0-4/P0-5 的第一版实现：

- P0-1：Claude Code adapter 支持 `TEKON_OUTPUT_DIR`、`TEKON_ARTIFACT_MANIFEST`，真实 provider 产物通过 manifest 校验 schema 后写入 Artifact Store；缺少必需 artifact 时节点失败。
- P0-3：内置 workflow 改为 `commandRef` 引用 `.tekon/repo-profile.yaml`，CLI 新增 `workflow preflight` 展示 gate 命令解析结果，在缺失命令时提示 repo profile 写入位置和候选脚本命令，并支持普通 command gate 的显式 `notApplicable`。
...
- P0-2/P0-6/P0-7 的评估支撑：新增 core work usability evaluator 和 CLI `eval work-usability --samples`，把真实样本、真实 provider、真实 PR、security scan、worktree 隔离和远端副作用审批纳入可执行评估。
- P1 需求塑形入口：新增 core demand shape/approve/evaluate，CLI `demand shape/approve/show`、`run --demand-file`、`eval demand-shape`，以及 Web session token 保护的需求塑形/批准后运行入口。当前是确定性需求卡生成和人工批准，不声明为真实 PM 自动澄清。
- P1 Workflow 选择：新增 `test-improvement`、`docs-update`、`plan-only` 受控内置模板，需求塑形会推荐对应模板；CLI 新增 `workflow select` 和 `eval workflow-selection`，Web 模板选择器同步展示受控模板。当前是确定性模板选择，不声明为动态 workflow 自动规划。
...
仍未完成并继续保留为后续 P0/P1 工作：真实仓库端到端样本集本身、至少 2 次真实测试 PR 证据、生产级隔离和真实 provider 长期稳定性数据；审阅导航还需要真实 run 反馈继续打磨。
