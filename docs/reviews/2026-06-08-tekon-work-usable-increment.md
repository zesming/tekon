...
结论：本轮已补齐 repo profile、角色 prompt 注入、Claude Code adapter CLI 接线、真实 provider artifact manifest 入库、repo profile 驱动 gate、缺失命令修复引导和显式不适用语义、provider 快照恢复、真实 worktree 主执行路径、PR 准备包、人工批准后的 PR 创建、PR 创建后的远端 CI 状态证据和 watch 轮询、Web approval 自动继续、CLI/Web 审批摘要、Web 受控发起 run/prepare/create-pr、需求塑形、受控 workflow selection、语义验收证据、安全扫描、命令日志脱敏、artifact 入库敏感信息拦截、工作就绪度评估、工作可用样本评估和第一版 CLI/Web 聚合审阅面。自动 merge、自动上线、动态 workflow 非 dry-run、生产级真实 LLM 稳定性、飞书 IM 通知和远程多租户服务仍未声明完成。

## 1. 背景判断

根据飞书方案和本地实现对照，Tekon 当前最应该补齐的是“人能放心拿去工作”的闭环，而不是扩展组织级全自动平台：
...
| 能力                         | 实现位置                                                                                                                       | 说明                                                                                                                                                                                                                                         |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 仓库画像                     | `packages/core/src/repo/profile.ts`                                                                                            | `init` 时根据 `package.json` 生成 `.tekon/repo-profile.yaml`，记录 build/typecheck/lint/test/e2e/security、PR base branch 和风险路径。                                                                                                       |
...
| Claude Code adapter CLI 接线 | `packages/cli/src/index.ts`、`packages/core/src/runtime/claude-code-adapter.ts`                                                | `run --agent claude-code` 会创建 Claude Code adapter；adapter 读取 `TEKON_ARTIFACT_MANIFEST`，校验 artifact schema 后写入 Artifact Store。                                                                                                   |
...
| 真实 worktree 主路径         | `packages/core/src/workflow/engine.ts`、`packages/core/src/runtime/worktree-manager.ts`                                        | CLI run/resume 注入 WorktreeManager；节点在 git worktree 中执行，Gate 在同一 worktree 中验证，通过后提交改动并推进到 `tekon-delivery/<runId>`，随后释放 lease。                                                                              |
...
| PR 准备包                    | `packages/core/src/delivery/pr-package.ts`、CLI `delivery prepare`                                                             | 生成 `.tekon/runs/<runId>/delivery/pr-package.md` 和 `pr-body.md`，记录 `delivery.pr-prepared` 审计事件，并汇总 acceptance/security evidence。                                                                                               |
| 受控 PR 创建                 | `packages/core/src/delivery/scm.ts`、CLI `delivery create-pr`                                                                  | 不带 `--approve-human` 只落库等待审批；带 `--approve-human` 后要求主工作区除 `.tekon` 外干净，直接 push 交付分支并执行 `gh pr create --body-file`，成功/失败/PR URL 均落库。                                                                 |
...
| 审批摘要                     | `packages/core/src/approval/summary.ts`、CLI `approval` / `eval approval-summary`、Web 待审批区                                | 为 pending human gate 生成可复制摘要，包含风险、命令、影响文件状态、readiness 失败项、证据入口和批准/拒绝/Web 处理入口；CLI 支持 `approval reject` 阻断 workflow。                                                                           |
...
npm exec --yes -- pnpm@10.12.1 --filter @tekon/web test
...
- `delivery/scm.test.ts`：PR 状态落库、人工审批、`.tekon` 排除、已有分支空 commit 跳过、PR URL 创建/恢复和失败阶段落库。
...
- `security/secrets.test.ts`：覆盖共享 secret scanner 的文本扫描、脱敏和忽略 `.tekon` 运行态目录。
...
- `approval/summary.test.ts` / `gate/human-gate.test.ts`：覆盖审批摘要生成、摘要评估、批准/拒绝入口和拒绝后 workflow 阻断。
- `cli` run-cli unit：覆盖 `approval summary`、`eval approval-summary` 和 `approval reject`。
- `web` API/e2e：待审批 API 和 dashboard 展示同一审批摘要，包含批准和拒绝入口。
...
- 团队级多项目权限、成本控制、飞书 IM 通知和长期知识沉淀仍是后续阶段能力；当前新增的是可复制审批摘要，不等于消息通知机器人已接入。
...
第二轮 reviewer 结论仍为 CHANGES_REQUIRED，必须修复项为：执行 `gh` 前未校验 runId/path segment，恶意 run id 搭配 selector 可能让 CommandGateway 日志 outputDir 逃逸 `.tekon/runs`。已修复：`queryPullRequestCiStatus` 在执行 `gh` 前校验 run 存在和 safe path segment，默认 outputDir 使用安全 run segment，显式 outputDir 必须位于仓库 `.tekon` 内，并补充 unsafe run id / escaped outputDir 不执行 gateway 的回归测试。最终复查结论以 reviewer 返回为准。
