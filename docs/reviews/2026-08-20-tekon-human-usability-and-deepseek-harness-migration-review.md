...
- **已采纳并推进**：报告 §12 立即修复清单（P0×6 + P1×8）与 §10 六阶段迁移方向，均纳入执行。落地方案见 [历史方案 2026-08-20-harness-replatform-execution-plan.md](https://github.com/zesming/tekon/blob/31680f9bfd9424dd6466734ac97a8d172debfa42/docs/superpowers/plans/2026-08-20-harness-replatform-execution-plan.md)。
...
- **每阶段落地明细与验收证据**：见 [历史方案 2026-08-20-harness-replatform-execution-plan.md](https://github.com/zesming/tekon/blob/31680f9bfd9424dd6466734ac97a8d172debfa42/docs/superpowers/plans/2026-08-20-harness-replatform-execution-plan.md)，该方案已过一轮评审 + 维护方自检；各阶段完成后在 `CHANGELOG.md` 记录版本与验证结果。
...
### 0.6 工程视角批注（2026-08-24 追加，阶段 4 实现级摸底订正）

> 本节基于阶段 4 前三路 explorer 对当前代码的实测摸底（workflow/gate-delivery/CLI 三子系统），**订正 §0.5 L47 中两处被证伪的乐观判断**，并给出实现级最小机制与子步拆分。落地以本节为准。

- **订正一：delivery 不是"只需事件订阅接线"**。delivery 与 engine 解耦属实（`delivery/pre-pr-readiness.ts` 只读 repositories+audit+fs），但：① `readiness/*` 事件类型**在事件词汇表里根本不存在**（`session-contract.ts:119-161` 无此类），需先定义类型 + 生产者；② `createPr` 执行**不幂等**（`scm.ts` 无 already-created 短路，重复 git push + gh pr create），自动触发前必须加幂等守卫；③ event bus 只在 web 组合根（`root.ts:57`）创建，**CLI run 不产生任何 session 事件**，故事件驱动 delivery 在 CLI 会话化前只对 web run 生效。
- **订正二：gate"已有 registry"字面对、实质误导**。`gate/registry.ts` 有代码有测试，但**生产 5 个 `createGateEngine` 调用点无一传 registry**——registry 是死代码，生产走 `gate/engine.ts` 的 legacy if/else。更关键：gate 触发模型是 engine 在 **node 边界同步 pull**（`node-executor.ts:338-349`），"gate 订阅 artifact/tool/workflow events 自动触发"是**触发模型变更**，不是接线。最小可行形态：事件触发 gate 限定为 **validation-only 旁路判定**（结果落 `gate/result` 供 UI/delivery 消费，**不动 workflow 状态机**），node 边界同步 gate 保留为状态机权威路径，靠 `gate-runner.ts:211-234` 的 latest-result 短路防双跑。
- **workflow 降级：方案 A（内置单节点 `goal` 模板 + job kind 分发）**。"run"表即 `workflow_instances`，10 张治理表非空 FK 绑它，**绕不开**（改 schema 违背最小改动）。故把 `workflow_instances` 泛化为通用 "run" 表（schema 不动），把"workflow 模板"从**唯一编排**降为 **plan 的一种来源**：新增内置 `goal` 单节点模板，engine 零改动即复用 role_run/lease/artifact/dual-write/job/cancel/pause/session 全套；gate 作为可选治理挂在 goal node（空 gates=纯 agent goal，非空=goal+治理）——这正是"workflow 是 governance plugin 的一种"的语义落地。plugin 边界落在三处个位数文件：plan 来源、事件词汇（`kind:'workflow'` 硬编码于 `dual-write.ts:91`、`project.ts:261`）、job kind 分发。**头号陷阱**：新 job kind 若漏接分发会掉进默认分支 → 空 plan → `engine.ts:436-449` 秒写 `run.passed`，产生"什么都没跑却显示成功"的假通过，比报错更危险，须显式分发 + 测试锁定。
- **CLI 会话化：比 §0.5 判断简单一档**。CLI/web 已共用 `createWorkflowEngine`（已劈成 `prepareRun`+`executePreparedRun`），session/job/dual-write/bus 全已从 `@tekon/core` 导出，唯一 web 专属件 `createWorkflowJobExecutor` 零 web 耦合。最小形态：executor 移入 core + 抽 `SessionService.startRun`（web/CLI 共用）+ CLI 内嵌 jobRunner **await job 终态再退出**（保住"跑完即退出"语义与既有 CLI e2e 断言）。必改项：CLI pause/cancel 现直改 DB 状态，须改走 `jobRunner.requestPause/requestCancel`，否则子进程取消链断裂；CLI 须启用 write queue + dual-write 包装。
- **profiles：配置/策略层，非运行时重写**。`sessions.profile` 现为纯展示字段（4 处硬编码 `'human-web'`、零行为分支）。三 profile 差异（启用哪些 gate、自动推进策略、推进边界、操作面）在现有运行时都有挂钩点。**治理红线（依 CLAUDE.md「合入、上线和高危动作必须受控」「Iron Man suit 优先」）**：`autonomous-delivery` 只能自动推进 capability 类 gate（build/lint/schema/security）并自动 **prepare** delivery；**human-approval gate 与 PR 创建仍须人工**，不因 profile 削弱——这是硬约束（对应 §11"人类 UI 稀释硬门禁"风险），不是可配项。
- **Demand → clarification/plan flow：加两轮,非重写**。澄清原语（`generateClarifyingQuestions`/`updateDraftWithAnswers`）已在 core、仅 CLI draft-interactive 用；审批点（draft approve）已双侧强制。缺的是：① web/headless 的非 TTY 澄清通道（把问/答做成 session 内的 turn 事件）；② 独立的 plan 产物 + 计划审批点（当前最接近的是 shape 的 `recommendedTemplate`+`acceptanceCriteria`，无独立 plan gate）。
- **子步拆分（每步独立 e2e，顺序含依赖）**：4a `SessionService` 抽取 + executor 移 core（web 行为不变的纯重构，为 CLI/goal 铺底）；4b workflow 降级为 goal plugin（goal 模板 + kind 分发 + 事件词汇）；4c CLI 会话化（走 SessionService + 内嵌 jobRunner + pause/cancel 改造）；4d profiles 策略层（含 autonomous-delivery 的受控边界）；4e Gate/Delivery 事件订阅（validation-only 旁路 gate + `readiness/evaluated` + 幂等守卫）；4f Demand→clarification/plan flow。legacy 清理（deprecated `demand.*` 四层别名）可在 4c/4f 顺带。范围大，若单轮设计过重可先交付 4a-4c（会话化闭环）再评估 4d-4f。
