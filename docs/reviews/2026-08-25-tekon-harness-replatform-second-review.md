...
- **S9（本轮已补）**：新增 `engine-gate-repair.e2e.test.ts`「repair 阶段被 fence」用例——autoFix build gate 持续失败驱动 repair 循环，fence 落在 repair 后，覆盖 repair-loop 顶部检查 (b) 与 exhausted-settle 前检查 (c)（`gate-runner.ts`）；已验证移除 (b)/(c) 守卫该测试即失败。原 gates-fence e2e 只驱动 (a)（首个 gate 失败即 return），(b)/(c)/M1/M2 catch 抛错路径此前未被单独锁——(b)/(c) 现已锁；M1/M2 catch 的抛错变体（gate/finalize 抛异常）仍未单独驱动，留待后续。
