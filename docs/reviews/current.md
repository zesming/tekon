...
- **当前报告**：[2026-09-03 Tekon 产品、UI/UX、Runtime 与 DeepSeek Harness 第十九轮全面复审](2026-09-03-tekon-product-runtime-harness-nineteenth-review.md)
...
- 全仓复核后发现 Advanced Run 的网络表述、提交门、mock 身份，以及 DSH telemetry 默认仍可独立修复；相关代码已提交并通过 Core #412 / CI #321。
...
2. **Advanced Run fail-closed**  
   无 token、计划加载中、plan error、workflow digest 缺失或网络确认缺失时，提交按钮保持禁用；handler 也保留防御式检查。

3. **mock Provider 产品身份**  
   Web 高级表单明确标注 mock 只用于测试/演示、生成合成结果、不执行真实任务、不能作为交付证据。

4. **DSH telemetry**  
   Headless child env 固定 `DSH_TELEMETRY_DISABLED=1`，不继承 ambient telemetry mode/endpoint；新增 exact-env 回归测试。
...
### #18 Run/Session 生命周期清理

`project.clean` 当前可直接删除 `.tekon/runs/<runId>`，但没有 non-terminal/active Job guard、Audit/tombstone、数据库路径修复、导出前置或失败恢复。

已扩展 [#18](https://github.com/zesming/tekon/issues/18)，把 complete export、compaction、retention 与 lifecycle-safe purge 统一处理。
...
- DSH child telemetry 默认；
- 顶栏 DSH unavailable 的 accessible description；
- mock Provider 在高级 Web 表单中无提示伪装成真实执行；
- 当前 Reviewer 修复快照的 Core、Root、Audit、CLI、Web unit 与 Chromium Playwright 门禁。
...
- **长 Session 与清理（#18）**：完整导出、模型 compaction、统一 retention、lifecycle-safe purge 和故障矩阵仍缺；
