# Tekon V2 Phase 1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a TypeScript monorepo CLI tool (`tekon`) that takes a natural language requirement and autonomously drives it through clarification→implementation→testing→review→PR delivery, using Claude Code as the underlying Agent.
...
│   │   │   ├── config.ts         # tekon.config.yaml loader
│   │   │   ├── workflow/
...
│   │   │   │   ├── init.ts       # tekon init
│   │   │   │   ├── run.ts        # tekon run
│   │   │   │   ├── status.ts     # tekon status
│   │   │   │   ├── role.ts       # tekon role *
│   │   │   │   ├── workflow.ts   # tekon workflow *
│   │   │   │   ├── constraints.ts
│   │   │   │   ├── log.ts
│   │   │   │   └── clean.ts
│   │   │   └── ui/
│   │   │       ├── status-view.tsx   # ink component for tekon status
...
export function getDbPath(projectRoot: string): string {
  return path.join(projectRoot, '.tekon', 'tekon.db');
}
...
describe('Role Loader', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tekon-test-'));
...
describe('Role Runner', () => {
  it('should spawn agent and collect output', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tekon-runner-'));
    ...
    const opts: AgentRunOptions = {
      ...
      outputDir,
      worktreePath: tmpDir,
      env: { TEKON_OUTPUT_DIR: outputDir, TEKON_ROLE: 'pm' },
    };
  ...
  it('should handle agent timeout', async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tekon-timeout-'));
    const outputDir = path.join(tmpDir, 'output');
    fs.mkdirSync(outputDir, { recursive: true });
...
export async function runAgent(opts: AgentRunOptions): Promise<AgentResult> {
  ...
  const procEnv = {
    ...
    ...env,
    TEKON_OUTPUT_DIR: outputDir,
    TEKON_ROLE: opts.role,
  };
...
export class WorkflowEngine {
  ...
  async execute(
    ...
    // Create output dir
    const outputBase = path.join(repoPath, '.tekon', 'runs', instance.id);
...
describe('ArtifactStore', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tekon-artifact-'));
...
export function initCommand(): Command {
  return new Command('init')
    .description('初始化当前目录为 Tekon 项目')
    .option('--repo <path>', '目标仓库路径', '.')
    .action(async (opts) => {
      const repoPath = path.resolve(opts.repo);
      const tekonDir = path.join(repoPath, '.tekon');
      const rolesDir = path.join(tekonDir, 'roles');

      if (!fs.existsSync(tekonDir)) {
        fs.mkdirSync(tekonDir, { recursive: true });
      }
      if (!fs.existsSync(rolesDir)) {
        fs.mkdirSync(rolesDir, { recursive: true });
      }

      const configPath = path.join(tekonDir, 'config.yaml');
...
`,
      ...
      console.log(`✓ Tekon 项目已初始化: ${tekonDir}`);
...
describe('tekon init', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tekon-init-'));
  });
  afterEach(() => {
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });

  it('should create .tekon directory and config', () => {
    execSync(
    ...
    expect(fs.existsSync(path.join(tmpDir, '.tekon'))).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, '.tekon', 'roles'))).toBe(true);
    expect(fs.existsSync(path.join(tmpDir, '.tekon', 'config.yaml'))).toBe(
      true,
    );
  });
});
...
import { Command } from 'commander';
import { WorkflowEngine } from '@tekon/core';
import { loadRoles } from '@tekon/core';
import { parseTemplate, listTemplates } from '@tekon/core';
import { runAgent } from '@tekon/core';
import path from 'node:path';
  ...
  return new Command('run')
    ...
    .action(async (description, opts) => {
      ...
      const engine = new WorkflowEngine({
        dbPath: path.join(tekonDir, 'tekon.db'),
      });
...
import { Command } from 'commander';
import { getDb, initDb, getDbPath } from '@tekon/core';
import path from 'node:path';
  ...
  return new Command('status')
    ...
    .action(async (opts) => {
      ...
      if (projects.length === 0) {
        console.log('No projects yet. Run `tekon run "..."` to start.');
        return;
      }
...
import { initDb, getDb, getDbPath } from '@tekon/core';
      ...
      <body>
        <div className="flex h-screen">
          <aside className="w-64 border-r bg-gray-50 p-4">
            <h1 className="text-lg font-bold mb-6">Tekon</h1>
...
export default async function HomePage() {
  ...
  return (
    <div>
      ...
      {projects.length === 0 && (
        <p className="text-gray-500">暂无项目。运行 `tekon run` 开始。</p>
      )}
      {projects.map((p: any) => (
...
import { WorkflowEngine } from '@tekon/core';
import { loadRoles } from '@tekon/core';
import { parseTemplate } from '@tekon/core';
import path from 'node:path';
import fs from 'node:fs';
import os from 'node:os';
...
describe('E2E: Full workflow execution', () => {
  it('should execute standard-feature workflow end-to-end', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'tekon-e2e-'));
    const rolesDir = path.join(tmpDir, '.tekon', 'roles');
    fs.mkdirSync(rolesDir, { recursive: true });
    ...
    // Execute
    const engine = new WorkflowEngine({
      dbPath: path.join(tmpDir, '.tekon', 'tekon.db'),
    });
...
### Task 20: Dogfooding — run Tekon on Tekon

- [ ] Set up Tekon project with `tekon init`
- [ ] Run one real feature request through Tekon to validate the flow
...
- [ ] `tekon init` creates .tekon structure
- [ ] `tekon run` executes a complete workflow with mock runner
- [ ] `tekon status` shows project state
- [ ] `tekon role list` shows all 5 roles with source labels
- [ ] `tekon constraints show` displays constraint rules
