...
├── docs/manual/tekon-mvp-user-manual.md
├── docs/manual/tekon-mvp-user-manual.html
...
└── docs/technical/tekon-v2-technical-plan.html
```
...
- 修改：`docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md`
- 修改：`docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html`
...
jobs:
  ...
  core:
    ...
    steps:
      ...
      - name: Build core
        run: npm exec --yes -- pnpm@10.12.1 --filter @tekon/core build

      - name: Run core unit tests
        run: npm exec --yes -- pnpm@10.12.1 --filter @tekon/core test:unit

      - name: Run core e2e tests
        run: npm exec --yes -- pnpm@10.12.1 --filter @tekon/core test:e2e
```
...
```bash
git add .github/workflows/core.yml package.json scripts/ci/actionlint.sh scripts/ci/check-github-actions.mjs docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html
...
- 创建：`README.md`
- 创建：`CHANGELOG.md`
- 创建：`docs/manual/tekon-mvp-user-manual.md`
- 创建：`docs/manual/tekon-mvp-user-manual.html`
- 修改：`docs/superpowers/plans/2026-06-05-tekon-v2-three-phase-implementation.md`
- 修改：`docs/superpowers/plans/2026-06-05-tekon-v2-three-phase-implementation.html`
- 修改：`docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md`
- 修改：`docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html`
...
Tekon V2 是本地 Agent workflow 系统的重构分支。当前 `rebuild-v2` 已完成阶段一 core 内核；还没有面向终端用户的 CLI 或 Web 产品入口。
...
- 暂不可用：`tekon` CLI 命令、Web dashboard、自动 PR 创建、动态 workflow 产品流、面向用户的项目初始化入口。
...
- 三阶段计划：`docs/superpowers/plans/2026-06-05-tekon-v2-three-phase-implementation.html`
- 阶段一评估：`docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html`
- 当前 MVP 边界：`docs/manual/tekon-mvp-user-manual.html`
````
...
- Tekon 尚未发布面向终端用户的 CLI、Web dashboard 或自动 PR 创建流程。
```
...
当前阶段只提供 `packages/core` 内核 API 和本地验证命令。普通用户还不能通过 `tekon` 命令发起需求、查看状态、打开 Web dashboard 或创建 PR。
...
- 不能运行 `tekon init`、`tekon run`、`tekon status`、`tekon pause`、`tekon resume`。
- 不能通过 Tekon 创建远端 PR。
- 不能打开 Tekon Web dashboard。
...
创建同章节的 `docs/manual/tekon-mvp-user-manual.html`，不得加入当前不存在的产品 quickstart。
...
docs/superpowers/plans/2026-06-05-tekon-v2-three-phase-implementation.md
docs/superpowers/plans/2026-06-05-tekon-v2-three-phase-implementation.html
docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md
docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html
```
...
npm exec --yes -- prettier --check README.md CHANGELOG.md docs/manual/tekon-mvp-user-manual.md docs/manual/tekon-mvp-user-manual.html vitest.config.ts package.json
test -f README.md
test -f CHANGELOG.md
test -f docs/manual/tekon-mvp-user-manual.html
```

运行未完成标记扫描；出现任何输出均失败。扫描范围只包含本任务预期新增或修改的交付物：

```bash
if rg -n "TB[D]|TO[D]O|FIXM[E]|placeholde[r]|待[补]|占[位]" README.md CHANGELOG.md docs/manual/tekon-mvp-user-manual.md docs/manual/tekon-mvp-user-manual.html docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html package.json packages/core vitest.config.ts; then
  exit 1
fi
```
...
```bash
if rg -n "vitest[.]workspace[.]ts|defineWorkspac[e]" package.json packages docs/manual docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html docs/superpowers/plans/2026-06-05-tekon-v2-three-phase-implementation.md docs/superpowers/plans/2026-06-05-tekon-v2-three-phase-implementation.html vitest.config.ts; then
  exit 1
fi
```
...
```bash
git add README.md CHANGELOG.md docs/manual/tekon-mvp-user-manual.md docs/manual/tekon-mvp-user-manual.html vitest.config.ts package.json docs/superpowers/plans/2026-06-05-tekon-v2-three-phase-implementation.md docs/superpowers/plans/2026-06-05-tekon-v2-three-phase-implementation.html docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html
...
describe('command gateway environment boundary', () => {
  ...
  it('supports exact env for manual provider smoke', async () => {
    ...
    expect(receivedEnv).toEqual({ PATH: '/usr/bin', HOME: '/tmp/tekon-home' });
  });
});
```
...
```bash
npm exec --yes -- pnpm@10.12.1 --filter @tekon/core test -- __tests__/runtime/command-gateway-env.test.ts --run
```
...
describe('claude-code provider manual smoke', () => {
  it('requires explicit enablement and authenticated Claude CLI', async () => {
    if (process.env.TEKON_CLAUDE_PROVIDER_SMOKE !== '1') {
      throw new Error(
        'TEKON_CLAUDE_PROVIDER_SMOKE=1 is required; this smoke is fail-closed.',
      );
    }
...
{
  "scripts": {
    "smoke:claude-provider": "pnpm --filter @tekon/core smoke:claude-provider"
  }
}
```
...
```bash
TEKON_CLAUDE_PROVIDER_SMOKE=1 \
TEKON_CLAUDE_COMMAND=claude \
...
- command: `TEKON_CLAUDE_PROVIDER_SMOKE=1 TEKON_CLAUDE_COMMAND=claude npm run smoke:claude-provider`
- exit code: 0
...
```bash
TEKON_CLAUDE_PROVIDER_SMOKE=1 TEKON_CLAUDE_COMMAND=claude npm run smoke:claude-provider
```
...
- 修改：`docs/technical/tekon-v2-technical-plan.md`
- 修改：`docs/technical/tekon-v2-technical-plan.html`
- 修改：`docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md`
- 修改：`docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html`
...
describe('command gateway network policy', () => {
  it.each([
    ...
    async (command) => {
      const cwd = mkdtempSync(join(tmpdir(), 'tekon-network-'));
      let spawnCalls = 0;
...
```bash
npm exec --yes -- pnpm@10.12.1 --filter @tekon/core test -- __tests__/runtime/command-gateway-network.test.ts --run
```
...
).toThrow('permission mode is controlled by Tekon');
...
```bash
npm exec --yes -- pnpm@10.12.1 --filter @tekon/core test -- __tests__/runtime/command-gateway-network.test.ts --run
npm exec --yes -- pnpm@10.12.1 --filter @tekon/core test -- __tests__/runtime/agent-adapter.test.ts __tests__/runtime/claude-code-adapter.test.ts --run
npm exec --yes -- pnpm@10.12.1 --filter @tekon/core test:unit
npm exec --yes -- pnpm@10.12.1 --filter @tekon/core test:e2e
```
...
```bash
if rg -n "bypassPermissions" docs/technical/tekon-v2-technical-plan.md docs/technical/tekon-v2-technical-plan.html docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html packages/core; then
...
```bash
git add packages/core/src/runtime/command-gateway.ts packages/core/src/runtime/agent-adapter.ts packages/core/src/runtime/claude-code-adapter.ts packages/core/__tests__/runtime/command-gateway-network.test.ts packages/core/__tests__/runtime/agent-adapter.test.ts packages/core/__tests__/runtime/claude-code-adapter.test.ts docs/technical/tekon-v2-technical-plan.md docs/technical/tekon-v2-technical-plan.html docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html
...
```bash
if rg -n "TB[D]|TO[D]O|FIXM[E]|placeholde[r]|待[补]|占[位]" README.md CHANGELOG.md docs/manual/tekon-mvp-user-manual.md docs/manual/tekon-mvp-user-manual.html docs/manual/claude-provider-smoke.md docs/manual/claude-provider-smoke.html docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.md docs/reviews/2026-06-05-tekon-v2-phase1-kernel-evaluation.html docs/reviews/2026-06-05-claude-provider-smoke-evidence.md docs/reviews/2026-06-05-claude-provider-smoke-evidence.html docs/reviews/2026-06-05-release-readiness-hardening-report.md docs/reviews/2026-06-05-release-readiness-hardening-report.html packages/core .github/workflows/core.yml scripts/ci package.json pnpm-workspace.yaml vitest.config.ts; then
  exit 1
fi
```
...
```bash
gh run list --repo zesming/tekon --workflow core.yml --branch rebuild-v2 --commit "$CODE_COMPLETE_SHA" --limit 10
...
```bash
gh run list --repo zesming/tekon --workflow core.yml --branch rebuild-v2 --commit "$REPORT_SHA" --limit 10
gh run view "$RUN_ID" --log-failed
```
