...
| OpenAI Codex 官方手册：`https://developers.openai.com/codex/agent-approvals-security` | Codex CLI 支持 `workspace-write`、`on-request`、默认无网络、危险全访问需要显式开启                                | Tekon 默认应使用 `codex --sandbox workspace-write --ask-for-approval on-request exec`，并拒绝用户绕过 sandbox/approval |
...
- Codex provider 默认命令使用 `codex --sandbox workspace-write --ask-for-approval on-request exec`。
- 用户配置不得覆盖 `--sandbox`、`--ask-for-approval`，不得开启 `danger-full-access` 或 bypass approvals。
