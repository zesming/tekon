...
推荐产品化目标架构。当前本地版本已落地 CLI、TUI、Local Runner、规则化 Gate、Coding Agent Adapter、本地测试验收和证据包子集；图中的 Console、持久化服务、CI Runner 和 PR 联动仍为后续阶段能力。
...
| 目标阶段选择 | 当前自动决定推进到需求文档、技术方案、任务拆解、开发执行、测试验收或风险报告；PR 交付为 Phase 2+ 能力 |
...
| 自动执行 | 当前通过 Local Runner 调用 allowlist 内的本地命令和可配置 Coding Agent Adapter |
...
判断：Donkey 不应复制 Coding Agent 的能力，而应提供统一 Adapter，把“目标阶段、上下文、仓库画像、验收标准、风险边界、证据要求”稳定交给外部执行者，并把执行结果归一成 Donkey 能管理的证据和状态。当前版本已提供 Codex / Claude / 自定义命令 Adapter；后续再增强隔离、队列、PR 和 CI 联动。
...
| 已有技术方案 | “按这个方案执行” | 方案校验 | 当前：测试验收、任务拆解或开发执行；后续：PR |
| 已有任务列表 | “这些任务帮我分派执行” | 任务校验 | 当前：任务拆解、开发执行或测试验收 |
...
| 开发执行 | 当前：Bugfix、小优化、研发效能工具增强、技术平台普通增强 | 自动调用配置的 Coding Agent Adapter 或 Runner 修改代码 |
...
sequenceDiagram
  ...
  WF->>Runner: 执行需要落地的节点
  Runner->>Tool: 调用 CLI / Skill / API / SDK / Coding Agent Adapter
...
| “这是技术方案，直接执行” | 校验方案缺口和风险；需要开发时调用 Coding Agent Adapter 修改代码，再进入本地测试验收 |
...
当前至少需要维护 fixture repos 和真实试点仓库两类 E2E；真实 PR 读取、CI 联动和稳定自动修复场景归入 Phase 2+。
...
- 10-20 个历史 B/D 类任务。
- 当前覆盖需求文档、技术方案、任务拆解、开发执行、本地测试验收和高危降级；Phase 2+ 扩展 PR review、CI 联动和团队级评测。
...
| MVP 入口 | 当前本地版本提供 CLI、短命令和 TUI；Console 为 Phase 2+ 后续辅助 | 最快接入真实仓库和研发流程，并降低反复输入长命令的成本 |
...
| Coding Agent | 通过 Adapter 接外部 Agent | 行业已有成熟代码执行者，Donkey 不应重复造轮子 |
...
8. 继续验证第一批 Coding Agent Adapter：建议 Codex + Claude Code 二选一或并行验证。
