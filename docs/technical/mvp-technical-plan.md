...
Donkey MVP 建议定位为“目标阶段驱动的 AI 自动交付运行时”，而不是固定从需求跑到 PR 的单一路径。本文中的“PR 交付”是后续 Phase 2+ 能力；当前 0.1.0 本地 MVP 只实现目标阶段判断、文档草案、本地测试验收、风险报告和证据包。
...
- **工作流**采用可裁剪 Workflow。系统先判断用户输入的上下文成熟度和目标阶段，再自动推进到最合适的终点，例如需求文档、技术方案、任务拆解、测试验收、风险报告；PR 交付属于 Phase 2+。
...
推荐产品化目标架构。当前 0.1.0 只落地 CLI、Local Runner、规则化 Gate、本地测试验收和证据包子集；图中的 Coding Agent Adapter、Console、持久化服务和 PR 联动均为后续阶段能力。
...
flowchart TB
  ...
  R -. Phase 2+ .-> CA[Coding Agent Adapter\nCodex / Claude Code / Cursor / Devin]
...
Donkey 的目标 MVP 要支撑 2-3 个相似技术基建仓库，覆盖 B/D 类需求的多种输入形态，并自动推进到合理目标阶段。本文后续凡涉及 Coding Agent、开发执行、真实 PR 读取、CI 联动和自动修复的内容，均为 Phase 2+ 或产品化目标，不代表当前 0.1.0 已实现。

阶段口径：

- Phase 1 先做单仓库、多目标阶段闭环。
- Phase 2 验证 2-3 个相似仓库复用，不把系统做成单仓库脚本。
- MVP 成功不以“是否创建 PR”为唯一标准，而是看“是否按输入成熟度推进到了正确阶段，并提供可验收证据”；创建 PR 是 Phase 2+ 的增量目标。
...
| 目标阶段选择 | 当前 0.1.0 自动决定推进到需求文档、技术方案、任务拆解、测试验收或风险报告；执行开发和 PR 交付为 Phase 2+ 能力 |
...
| 自动执行 | 当前 0.1.0 通过 Local Runner 调用 allowlist 内的本地命令；外部 Coding Agent Adapter 为 Phase 2+ 能力 |
| 自动验证 | 当前 0.1.0 消费本地测试命令结果并生成验收证据；CI、E2E 和静态检查联动逐步接入 |
...
判断：Donkey 不应在当前 0.1.0 复制或直接接入 Coding Agent。Phase 2+ 再提供统一 Adapter，把“目标阶段、上下文、仓库画像、验收标准、风险边界、证据要求”稳定交给外部执行者，并把执行结果归一成 Donkey 能管理的证据和状态。
...
| 组件 | 职责 | 阶段口径 |
|-|-|-|
| Donkey CLI | 创建运行、传入已有文档/方案/PR 内容或变更说明、查看状态、触发测试、导出证据包 | 当前 0.1.0 已实现最小入口 |
...
| Donkey Console | 展示运行状态、证据包、风险确认、最终验收和退回 | Phase 1 后续可先做轻量 Web 页面，再接飞书通知 |
...
| 已有技术方案 | “按这个方案执行” | 方案校验 | 当前 0.1.0：测试验收或任务拆解；后续：开发执行或 PR |
| 已有任务列表 | “这些任务帮我分派执行” | 任务校验 | 当前 0.1.0：任务拆解或测试验收；Phase 2+：开发执行 |
| 已有代码变更 | “帮我测试验收” | Diff/变更说明分析 | 验收证据包 |
| 已有 PR | “审一下这个 PR” | 当前 0.1.0：基于用户提供内容生成验收判断；Phase 2+：真实 PR 上下文读取 | Review 报告或验收结论 |
...
| 开发执行 | Phase 2+：Bugfix、小优化、研发效能工具增强、技术平台普通增强 | 自动调用 Coding Agent 或 Runner 修改代码 |
| 测试验收 | 当前 0.1.0：已有技术方案、代码变更说明或已有执行结果；Phase 2+：真实 PR 和 CI 上下文 | 自动运行测试、E2E、静态检查，生成证据包 |
| PR 交付 | Phase 2+：代码变更安全、测试可执行、无高危动作 | 自动创建 PR 和交付证据包 |
...
targetStages:
  - validation_report
  # pull_request 属于 Phase 2+，当前 0.1.0 不创建 PR。
stages:
  - id: context_check
    nodes:
  ...
  - id: execution
    phase: Phase 2+
    skipWhen: currentVersion == "0.1.0" or (targetStage == "validation_report" and noCodeChangeRequired)
    nodes:
...
version: 0.1.0
role: Test
description: 当前 0.1.0 在已有需求、技术方案、代码变更说明基础上选择并运行必要测试；Phase 2+ 可读取真实 PR 与 CI 上下文，生成可验收证据。
responsibilities:
  - 根据 Repo Profile 选择单测、lint、类型检查、E2E 或 CI 查询
  - 当前 0.1.0 对失败进行分类并报告；Phase 2+ 判断是否适合自动修复
...
failurePolicy:
  maxAutoFixAttempts: 0 # 当前 0.1.0 只分类并报告失败；Phase 2+ 可提升为 2-3。
...
| RD Agent | Phase 2+ 调用 Coding Agent 或 Runner 执行代码修改；当前 0.1.0 仅保留角色画像 | 隔离分支写代码、运行限定命令 |
...
sequenceDiagram
  ...
  WF->>Runner: 执行需要落地的节点
  Runner->>Tool: 当前调用 CLI / Skill / API / SDK；Phase 2+ 调用外部 Coding Agent
  Tool-->>Runner: 结果 / 日志 / diff / 测试产物
  Runner-->>Evidence: 归档日志、trace、截图、报告
  Evidence-->>Donkey: 生成目标阶段交付物
  Donkey-->>User: 需求文档 / 技术方案 / 测试验收 / 风险报告 / Phase 2+ PR
```
...
| “这是技术方案，直接执行” | 当前 0.1.0 校验方案缺口和风险，直接进入任务拆解或本地测试验收；Phase 2+ 才进入开发执行 |
| “这个 PR 帮我验收” | 当前 0.1.0 可基于用户提供的变更说明运行本地验收；真实 PR、diff 和 CI 读取归入 Phase 2+ |
...
| 测试 workflow / lint / 非发布类 CI 优化 | 中风险；当前 0.1.0 先输出测试验收或风险说明，Phase 2+ 可自动到 PR 并在交付包突出风险 |
...
| 边界 | 要求 |
|-|-|
| 执行环境 | 当前 0.1.0 只运行 allowlist 内本地命令；Phase 2+ 外部 Coding Agent 或自托管 Runner 必须运行在临时沙箱、容器或隔离 worktree 中 |
...
| 重试上限 | 当前 0.1.0 只报告失败；Phase 2+ 同一验证失败最多自动修复 2-3 轮，超过后升级为阻塞报告 |
| 外部 Agent 超时 | Phase 2+：Coding Agent 执行超过阈值后取消或转人工确认 |
...
当前 0.1.0 至少需要维护 fixture repos 和真实试点仓库两类 E2E；涉及自动代码修改、真实 PR 读取和自动修复的场景归入 Phase 2+。

| 场景 | 阶段 | 期望 |
|-|-|-|
| 从想法到需求文档 | 当前 0.1.0 | 自动生成需求文档、验收标准、缺口列表 |
| 从需求到技术方案 | 当前 0.1.0 | 自动生成技术方案、风险、任务拆解 |
| 从技术方案到测试验收 | 当前 0.1.0 | 跳过需求阶段，运行本地测试命令，输出证据包 |
| 从已有代码变更说明到验收报告 | 当前 0.1.0 扩展 | 基于用户提供的变更说明运行测试，输出证据包和 reviewer 关注点 |
| 从真实 PR 到验收报告 | Phase 2+ | 读取 PR、diff、CI，运行测试，输出证据包和 reviewer 关注点 |
| 测试失败自动修复 | Phase 2+ | 失败分类、自动修复、重跑测试，超过上限后升级 |
| 高危需求降级 | 当前 0.1.0 | 生产/权限/密钥类需求不执行危险动作，只输出方案和清单 |
...
| RD Agent | Phase 2+ 指标：代码变更成功率、测试通过率、人工返工率 |
...
- 10-20 个历史 B/D 类任务。
- 当前 0.1.0 覆盖需求文档、技术方案、任务拆解、本地测试验收和高危降级；Phase 2+ 扩展开发执行和 PR review。
...
| 自动到 PR 比例 | Phase 2+ 指标：对适合 PR 的需求，50%+ 无中途人工介入跑到 PR |
| 交付周期 | 较同类人工流程中位数下降 30%+ |
| 验收标准可判定率 | 90%+ |
| PR 一次审查通过率 | Phase 2+ 指标：60%+ |
| 高危动作误执行 | 0 |
| 自动合入/自动上线 | 0 |
...
- 哪个工具被调用过，权限策略是什么。
- 哪些测试跑过，结果是什么。
- 当前 0.1.0 哪些失败被报告和阻断；Phase 2+ 哪些失败被自动修复了。
- 为什么建议接受、退回或停止在当前阶段。
...
- Local Runner 能完成“已有技术方案 -> 测试验收报告”或“已有代码变更说明 -> 验收报告”；真实 PR 读取与联动归入 Phase 2+。
- 高危动作策略能在测试中稳定拦截。
...
- 从想法到需求文档。
- 从需求到技术方案。
- 从技术方案到任务拆解或本地测试验收；开发执行归入 Phase 2+。
- 从已有代码变更说明到验收证据包；真实 PR 验收作为 Phase 2+ 扩展路径。
- 高危需求降级为方案和风险报告。

范围口径：Phase 1 至少跑通前三条路径；“已有代码变更说明到验收证据包”和“高危需求降级”作为扩展验收项。真实 PR 读取、CI 状态联动和 PR 验收归入 Phase 2+。
...
- Phase 2+：对适合 PR 的需求，50%+ 无中途人工介入跑到 PR。
- 交付周期较同类人工流程中位数下降 30%+。
...
| MVP 入口 | 当前 0.1.0 CLI 优先；Console 为 Phase 1 后续辅助 | 最快接入真实仓库和研发流程 |
...
| Coding Agent | Phase 2+ 通过 Adapter 接外部 Agent | 行业已有成熟代码执行者，Donkey 不应重复造轮子 |
...
| 执行隔离 | 当前 0.1.0 使用本地 allowlist 命令；Phase 2+ 使用临时沙箱/容器/隔离 runner + 分支隔离 | 降低代码修改、工具调用、网络访问和 secret 外泄风险 |
...
6. 设计 Workflow Definition schema，先支持 3 条核心路径：已有技术方案到本地测试验收、从想法到需求文档、从需求到技术方案；已有代码变更说明验收作为扩展路径补齐，真实 PR 验收放到 Phase 2+。
7. 实现 CLI + Local Runner，优先调用 CLI/Skill/API，不依赖 MCP。
8. Phase 2+ 前置调研第一批 Coding Agent Adapter：建议 Codex + Claude Code 二选一或并行验证。
