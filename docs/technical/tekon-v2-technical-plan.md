...
Tekon V2 是一个面向技术基建团队的 AI 自动交付系统。用户提出需求目标，Tekon 自动组织可协作的 AI 角色团队，通过动态 Workflow、角色工具链和质量 Gate，把项目从需求推进到可验收的 PR。
...
- Tekon 本身就是一个会被 AI 辅助开发的项目，技术栈应该对自己最友好
...
`tools.yaml` 定义该角色可调用的外部工具及允许的命令白名单。Engine 在 spawn Agent 子进程时将 `tools.yaml` 内容渲染为人类可读的 Markdown 片段，注入到 system prompt 的 `{{loaded_tools}}` 变量位置。危险命令（如 `rm -rf`、`git push --force`）不在白名单中时，Agent 运行时会被 Tekon 的 Tool Gateway 拦截。
...
Tekon 控制 Claude Code 的 `--permission-mode`，角色配置和用户自定义 args 不允许注入 `--permission-mode`、`--dangerously-skip-permissions` 或任何包含 `bypassPermissions` 的参数。`bypassPermissions` 不作为 Tekon 默认或推荐模式。只有在外层 OS/container sandbox 已验证、HumanGate 已批准、并且证据报告记录隔离方式时，才允许作为受控实验配置。
...
| `file`       | 写入临时文件，通过环境变量 `TEKON_PROMPT_FILE` 传入路径 | 超长 prompt 超过 shell argv 限制时 |
...
**自定义命令的接管协议**：只要自定义脚本/CLI 能从 stdin 读取 prompt 并将产物写入 `TEKON_OUTPUT_DIR` 环境变量指定的目录，即可作为 Tekon Agent 使用。Engine 在 spawn 前设置以下环境变量：

| 环境变量            | 值                          | 说明                          |
| ------------------- | --------------------------- | ----------------------------- |
| `TEKON_OUTPUT_DIR`  | `<worktree>/.tekon-output/` | Agent 写入产物的目录          |
| `TEKON_ROLE`        | `pm` / `rd` / ...           | 当前角色名                    |
| `TEKON_PROJECT_ID`  | `proj-xxx`                  | 项目 ID                       |
| `TEKON_NODE_ID`     | `node-xxx`                  | 当前 Node ID                  |
| `TEKON_PROMPT_FILE` | 临时文件路径                | `file` 模式时指向 prompt 文件 |
...
```
1. 项目级：<project>/.tekon/roles/<roleName>/    ← 使用者自定义
2. 用户级：~/.tekon/roles/<roleName>/            ← 使用者全局自定义
3. 内置：  <tekon-pkg>/roles/<roleName>/         ← 开发者预置
...
1. 项目级 `<project>/.tekon/roles/<role>/skills/`
2. 用户级 `~/.tekon/roles/<role>/skills/`
3. 内置 `<tekon-pkg>/roles/<role>/skills/`
...
```
场景：
tekon run → PM 完成 → RD 执行中 → Ctrl+C
tekon run → Engine 读 SQLite → 从 RD 的 Node 继续
```
...
tekon run --dynamic --save-as batch-retry "给任务平台加批量重试"
```
...
```bash
# 交互式创建
tekon workflow create

# 从已有模板复制
tekon workflow create --from standard-feature --name my-custom

# 快速生成骨架
tekon workflow create --name api-dev --roles pm,rd,qa --preset api
```
...
Agent Runtime 是 Tekon 中最薄的一层。职责：角色配置 → 子进程调用 → 收集结果。
...
每个 worktree：
├── <原仓库代码>
└── .tekon-output/             # Agent 写入产物的目录
```

执行完后 Engine 从 `.tekon-output/` 读取产物，worktree 可清理或保留审计。
...
内置 preset（`claude-code` / `codex`）是预设的 command + args 组合，用户可替换 command 并追加安全 args，但 provider 权限模式由 Tekon 控制。只要符合"stdin prompt → stdout 产物"协议即可。
...
```bash
# ── 项目管理 ──
tekon init                    # 初始化当前目录为 Tekon 项目
tekon run "需求描述"           # 启动需求（默认动态模式）
tekon run --template <name>   # 使用指定模板
tekon run --dynamic           # 强制动态模式
tekon run --dry-run           # 预览 Workflow，不执行
tekon status                  # 查看当前项目状态
tekon pause                   # 暂停
tekon resume                  # 从中断点恢复
tekon cancel                  # 取消

# ── 角色管理 ──
tekon role list               # 列出所有角色（标注来源：内置/用户/项目）
tekon role show <name>        # 查看角色详情
tekon role path <name>        # 输出角色目录路径
tekon role create <name>      # 交互式创建角色

# ── Workflow 管理 ──
tekon workflow list           # 列出所有模板
tekon workflow create         # 交互式创建
tekon workflow create --from <name> --name <new>
tekon workflow show <name>    # 查看模板内容

# ── 约束管理 ──
tekon constraints show        # 查看约束规则
...
Phase 1 期间让 Tekon 管理 Tekon 自身的开发：
...
| 7-8 | 完整自举：Tekon 管理 Tekon 一个完整迭代     |
