...
import { projectRunPlan, loadWorkflowTemplate, openTekonDatabase } from '@tekon/core';
...
describe('CLI run plan digest and snapshot persistence', () => {
  ...
  it('calculates canonical plan digest for workflow run and completes', async () => {
    ...
    try {
      ...
      expect(
        (row as Record<string, unknown>).plan_snapshot,
      ).toBeTruthy();
    } finally {
      db.close();
    }
  ...
  it('calculates canonical plan digest for goal mode run and completes', async () => {
    const repoPath = createFixtureRepo(tempDirs);
    const io = createMemoryIo();
