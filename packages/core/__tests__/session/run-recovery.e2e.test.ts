...
setInterval(()=>{appendFileSync(log,'beat\\n');if(existsSync(log+'.release'))process.exit(0)},20);`);
  ...
  async function start(name: string, agent = false) {
    ...
    const workflowSpec: WorkflowTemplate = { id: name, name, version: 1,
      ...
      phases: [{ id: 'phase', name: 'Phase', dependsOn: [], parallel: false, nodes: [{ id: 'node', role: 'rd',
        inputs: [], outputs: [], dependsOn: [], gates: agent ? [] : [{ type: 'test', command: { tool: 'npm', args: ['run', 'heartbeat', '--', log] } }] }] }] };
...
executor:t.createWorkflowJobExecutor({repositories,sessions,bus,audit,registry,projectContext:{projectRoot:${JSON.stringify(root)}}})});
