  ...
  return {
    async runGate(input) {
      ...
      if (isCommandGate(input.gate.type)) {
        ...
        await createHumanGate({ repositories: options.repositories }).requestHumanGate({
          runId: input.runId,
          nodeId: input.nodeId,
...
function isCommandGate(type: GateConfig['type']): type is 'build' | 'test' | 'lint' | 'e2e-pass' | 'security-scan' {
